 ...
 * - 哔哩哔哩会员购《用户服务协议》《购前协议》
 * - SNKRDUNK《利用規約》与返品说明
 * 已剔除：魔力赏抽赏、C2C 鉴定仓全额补偿、二次付款合单等与本站模式不符的内容。
...
const TOC = [
  ...
  { id: "art-5", label: "05 支付" },
  { id: "art-6", label: "06 发货与签收" },
  { id: "art-7", label: "07 预售延期退款" },
  { id: "art-8", label: "08 特殊品类" },
  { id: "art-9", label: "09 售后与举证" },
  { id: "art-10", label: "10 开封结果" },
  { id: "art-11", label: "11 禁止行为" },
  { id: "art-12", label: "12 免责范围" },
  { id: "art-13", label: "13 协议变更" },
  { id: "art-14", label: "14 管辖法律" },
  { id: "art-15", label: "15 联系方式" },
];

const KEY_NOTICES = [
  {
    title: "下单即视为同意本协议",
    body: "浏览、加购、提交订单并完成付款，即视为已阅读、理解并同意本协议及当时有效规则。",
  },
  {
    title: "特殊品不适用笼统七天无理由",
  ...
  {
    title: "签收后 7 日内举证",
    body: "运输破损、错发、漏发等须在签收后 7 日内联系客服，并提供开箱视频与照片。",
  },
  {
    title: "预售满 90 日未发货可退",
    body: "自首次展示的预计发货日起满 90 日仍未发货，且订单尚未发货时，可按实付金额申请退款。",
  },
];
  ...
  return (
    ...
    >
      ...
      <LegalArticle id="art-2" index="02" title="账户与使用资格">
        <ol className="list-decimal space-y-2">
          <li>您应提供真实、准确、完整的注册与收货信息，并及时更新。</li>
          <li>因您提供错误地址、联系方式或拒不配合核实导致无法履约的，本站有权取消订单或暂停发货，相关损失由您承担。</li>
          <li>未成年人应在监护人同意下使用本站服务；监护人应对相关行为负责。</li>
          <li>本站有权在合理怀疑欺诈、滥用优惠、恶意退换、扰乱经营等情形时，限制账号、取消订单或拒绝交易。</li>
        </ol>
      </LegalArticle>

      <LegalArticle id="art-3" index="03" title="商品信息与价格">
        <ol className="list-decimal space-y-2">
          <li>
            商品名称、图片、价格、库存、发货说明以商品页及结账页展示为准。图片仅为示意，实物品相以 sealed /
            评级状态及页面说明为准。
          </li>
          <li>价格以日元（JPY）标示；税费是否含税以页面标注为准。结账金额以 Stripe 支付页最终显示为准。</li>
          <li>
            本站<strong>不对商品是否满足您的特定收藏目标、投资收益、转售利润或其他特定目的</strong>
            作任何明示或默示担保。
          </li>
          <li>二级市场行情仅供参考，不构成投资建议，亦不作为退换货依据。</li>
        </ol>
      </LegalArticle>
      ...
      <LegalArticle id="art-4" index="04" title="订单成立、缺货与取消">
        <ol className="list-decimal space-y-2">
          ...
          <li>
            付款完成后，除本协议明确列明的可退款情形外，
            <strong>原则上不支持退款</strong>。
          </li>
        </ol>
      </LegalArticle>

      <LegalArticle id="art-5" index="05" title="支付">
        <ol className="list-decimal space-y-2">
          <li>支付方式：信用卡等由 Stripe 提供的方式（Visa / Mastercard / JCB 等，以结账页可选为准）。</li>
          <li>支付信息由 Stripe 处理；本站不存储完整卡号。</li>
      ...
      <LegalArticle id="art-6" index="06" title="发货、配送与签收">
        <ol className="list-decimal space-y-2">
          ...
          <li>
            ...
            ，可能因厂商出荷、质检返工、库存调配、清关、承运、目的地等因素产生偏差；实际发货时间以本站实际发货为准。您知悉并理解上述情形。
          </li>
          <li>运费以结账页为准。目的地关税、税费、清关手续费由买家承担；本站不垫付、不承诺免税通关。</li>
          <li>
            因海关查验、地址错误、长期无人签收、无正当理由拒签等导致的延误、退回、毁损灭失风险及费用，由买家承担。
            <LegalCallout tone="warn">
              您无正当理由拒绝签收不适用「七天无理由」或性质不宜拒签的商品的，商品返回后的风险与费用由您承担；本站可选择退款（可扣除实际损失）或再次发回；若不宜再发或您再次拒签，订单损失由您承担。
            </LegalCallout>
          </li>
          <li>请于签收时核对外包装；建议拍摄完整开箱视频备查。</li>
        </ol>
      </LegalArticle>
      ...
      <LegalArticle id="art-8" index="08" title="特殊品类：不适用笼统「七天无理由」">
        ...
        <p className="mt-2">
          不支持的个人原因包括但不限于：不喜欢、买多了、想换款、尺寸/款式预期不符、二级市场价格波动、开封后抽卡结果不理想等。
        </p>
      </LegalArticle>
      ...
      <LegalArticle id="art-9" index="09" title="售后范围、举证与处理">
        <ol className="list-decimal space-y-2">
          <li>
            <strong>可受理范围</strong>
            ：运输导致的明显外包装破损致商品实质受损、错发、漏发；须能合理证明问题在签收时即已存在。
          </li>
          <li>
            <strong>非售后范围（示例）</strong>
            ：上条个人原因；已拆封/已开封；外箱轻微压痕、角损、工厂原有轻微瑕疵且不影响 sealed
            完整性；您自行二次包装、转运、仓储不当造成的损坏；开封后的卡牌内容与期望不符。
          </li>
          <li>
            <LegalCallout>
              请于物流签收后 7 日内联系客服（{SUPPORT_EMAIL}）提出申请。逾期本站有权不予受理。
            </LegalCallout>
          </li>
          <li>
            <strong>举证</strong>
            ：须提供订单号、完整开箱视频（建议从封闭包裹连续拍到取出商品）、破损/错发清晰照片及其他本站要求的材料。无法提供有效凭证的，本站有权不予受理。
          </li>
          <li>
            <strong>处理方式</strong>
            ：经核实属本站责任的，本站可优先安排补发或换货；无合理库存时可退还该商品实付金额。来回运费承担以核实结果为准。本站不接受到付退货。退货商品在本站签收前的风险由您承担；您应在收到可寄回通知后
            7 日内寄出并提供单号，超时寄出的，本站有权对同一事由再次主张不予处理。
          </li>
          <li>
            本站不承诺「全额补偿鉴定」或鉴定仓级服务；正品承诺限于可溯源正规进货渠道，不等于第三方鉴定机构背书。
          </li>
        </ol>
      </LegalArticle>

      <LegalArticle id="art-10" index="10" title="随机性与开封结果">
        <ol className="list-decimal space-y-2">
          <li>集换式卡牌开封结果具有随机性，可能出现重复或不含您期望的特定卡牌。</li>
          <li>本站不保证特定卡牌、卡号、评级分数或二级市场价格，亦不对开封结果承担退款或赔偿义务。</li>
          <li>请理性购买；一经付款，除本协议特别约定外，不以「没抽到」为由要求退款。</li>
        </ol>
      </LegalArticle>

      <LegalArticle id="art-11" index="11" title="禁止行为">
        <p className="mb-2">您不得从事以下行为，否则本站有权采取限制账号、取消订单、追究责任等措施：</p>
        <ul className="list-disc space-y-1">
          <li>提供虚假信息、盗用他人身份或支付方式</li>
          <li>干扰网站运行、爬取滥用接口、绕过风控</li>
          <li>恶意拒付、虚假售后主张、重复纠缠同一已结案事由</li>
          <li>将本站用于违法目的或侵害第三方权益</li>
        </ul>
      </LegalArticle>

      <LegalArticle id="art-12" index="12" title="免责与责任范围">
        <ol className="list-decimal space-y-2">
          <li>
            因不可抗力、承运商或海关原因造成的延误、丢失或损坏，本站在已履行合理包装与发货义务的前提下，责任以承运商理赔范围及本协议约定为限。
          </li>
          <li>
            在法律允许的最大范围内，本站就本协议项下的赔偿责任以您就争议订单实际支付的商品价款为上限（不含运费、关税等第三方费用，法律另有强制规定的除外）。
          </li>
          <li>本站不对间接损失、预期利益、商誉损失承担责任。</li>
        </ol>
      </LegalArticle>

      <LegalArticle id="art-13" index="13" title="协议变更">
        <p>
          本站有权更新本协议。更新后公布于网站即生效；您于生效后继续下单，视为接受更新后的条款。重大变更将尽量通过网站提示告知。
        </p>
      </LegalArticle>

      <LegalArticle id="art-14" index="14" title="适用法律与管辖">
        <p>
          本协议受日本法律管辖。因本协议引起的争议，双方应先协商；协商不成的，提交日本东京地方法院为第一审专属合意管辖法院。
        </p>
      </LegalArticle>

      <LegalArticle id="art-15" index="15" title="联系方式">
        <div className="legal-info-card">
          ...
          <div className="legal-info-row">
            <span className="legal-info-label">客服</span>
          ...
          <div className="legal-info-row">
            <span className="legal-info-label">批发合作</span>
            <span className="legal-info-value">
              <a href={`mailto:${B2B_EMAIL}`}>{B2B_EMAIL}</a>
              {" ｜ "}
              <Link href="/wholesale">批发合作页</Link>
            </span>
          </div>
        </div>
      </LegalArticle>
